    ...
    IEnumerator Shoot()
    {
        ...
        switch (currentMode)
        {
            ...
            case FiringMode.Railgun:
                ...
                {
                    // Recalculate direction and angle right before firing each "particle"
                    Vector2 updatedDirection = (reticle.position - player.position).normalized;
                    float updatedAngle = Mathf.Atan2(updatedDirection.y, updatedDirection.x) * Mathf.Rad2Deg;

                    Instantiate(railgunBulletPrefab, reticle.position, Quaternion.Euler(0f, 0f, updatedAngle));
